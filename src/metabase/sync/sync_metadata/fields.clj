...
(s/defn ^:private ^:always-validate create-or-reactivate-fields!
  "Create new Field objects, or mark existing inactive ones as active, for a sequence of NEW-FIELDS metadata belonging to TABLE."
...
(s/defn ^:private ^:always-validate retire-fields!
  "Mark a sequence of OLD-FIELDS belonging to TABLE as inactive if corresponding Field objects exist."
...
(s/defn ^:private ^:always-validate our-metadata :- #{i/TableMetadataField}
  "Return information we have about Fields for a TABLE currently in the application database
   in the same `TableMetadataField` returned by `describe-table`."
